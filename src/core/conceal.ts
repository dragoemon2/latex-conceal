...
/**
 * テキスト全体からすべての数式環境の範囲を抽出する
 * @param text LaTeXのドキュメント全体のテキスト
 * @returns 数式環境の範囲のリスト (出現順)
 */
export function getAllMathEnvs(text: string): TextRange[] {
    const mathEnvs: TextRange[] = [];
    const envNames = 'equation|align|alignat|flalign|multline|gather|math|displaymath';

    // 1. \begin{指定した環境名 + オプションの*} ... \end{同じ環境名}
    //    ((?: ... )\*?) がキャプチャグループ1になり、閉じタグと完全一致させる
    // 2. \[ ... \]
    // 3. \( ... \)
    // 4. $$ ... $$
    // 5. $ ... $
    const MATH_ENV_REGEX = new RegExp(
